...
import { getProfileSeller } from '../API/APIGateway.js';
import { translations } from '../components/translations.js';
import "../styles/carrito.css";

export default function MisPublicaciones({ language, setBookOpen, setFromPurchased }) {
  ...
  useEffect(() => {
    const loadBooks = async () => {
      try {
        ...
        const profileData = await getProfileSeller(profileId);
        const booksIds = profileData.data.books || [];

        // Obtener detalles completos de cada libro publicado
  ...
  if (loading) {
    return (
      <div style={{ padding: '20px', textAlign: 'center' }}>
        <p>{translations[language].txt_cargando}</p>
      </div>
    );
  }
  ...
  return (
    <div className="carrito-container">
      <h2>{translations[language].txt_publicaciones}</h2>
      
      {books.length === 0 ? (
        <p className="text-center color-gray mt-20">
          {translations[language].txt_no_publicaciones}
        </p>
      ) : (
              ...
              <div className="item-card-content">
                ...
                <div className="item-card-footer">
                  <p style={{ margin: 0, fontSize: '14px', fontWeight: 'bold' }}>
                    {translations[language].txt_precio}: ${book.price?.toLocaleString("es-CL") || 0}
                  </p>
                  
                  <p style={{ margin: 0, fontSize: '14px', fontWeight: 'bold' }}>
                    {translations[language].txt_stock}: {book.stock}
                  </p>
